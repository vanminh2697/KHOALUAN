...
class WordPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def fit(self):
        ...
        file = open("vocab_word.txt",'r')
        string = file.read().split(' ')
        file.close()
        vocab_word_file = OrderedDict()
        for i in string:
            vocab_word_file[i] = len(vocab_word_file)
        
        file  = open("vocab_tag.txt",'r')
        string = file.read().split(' ')
        file.close()
        vocab_tag_file = OrderedDict()
        for i in string:
            vocab_tag_file[i] = len(vocab_tag_file)


        self.vocab_word = vocab_word_file
        self.vocab_tag = vocab_tag_file
